...
def train(epoch, model, path,dataloader, optimizer, device, patience = 10):
    model.to(device)
    best_loss = float('inf')
    best_rmse = float('inf')
    cur_patience = 0
...
def calc_acc(preds,labels):
    ...
    for p,l in zip(preds,labels):
        new_p, new_l = score_to_cat(p),score_to_cat(l)
        if new_p==new_l:
            correct_count+=1
...
#print('train start.')
path_to_load = './/models//'+args.model_name.split('/')[-1]+'//'+args.ability+'//'
...
eval_avg, eval_rmse,preds,labels = evaluate(model,path_to_load, test_loader, device)
...
logdict['acc'] = accuracy
with open("./logs/regression_eval_log.json", "r") as f:
    log = json.load(f)
...
with open("./logs/regression_eval_log.json", 'w') as f:
