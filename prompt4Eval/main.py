...
def train(model, optimizer, data_loader, device):
    model.to(device)
    model.train()
    mean_loss = torch.zeros(2).to(device)
    ...
    return loss_epoch.item(), acc.item()
...
for epoch in range(args.epochs):
    ...
    loss, acc_tra = train(net, optimizer, train_loader,device)
    losses.append(loss)
    acc_values.append(acc_tra)

    end_tra = time.time()
    train_spend = (end_tra - st_tra) / 3600
    print("Train Loss: %0.4f" % loss)
    print("Train ACC: %0.4f\t[%0.2f]" %
          (acc_tra,train_spend))
    if best_loss-loss>0.001:
        ...
        torch.save(net, save_path+f'model_{args.ability}.pt')
    else:
        cur_patience+=1
    if acc_tra>best_acc:
        best_acc = acc_tra
...
val_scores, val_loss,acc_val,predictions = eval(best_model, val_loader)
...
logdict['train_loss'] = best_loss
logdict['train_acc'] = best_acc
logdict['eval_loss'] = val_loss
logdict['eval_acc'] = acc_val
logdict['log_loss'] = losses
logdict['log_acc'] = acc_values
logdict['predict_result'] = predictions

with open("/home/work/deeptext/yys/redpen/logs/prompt_log.json", "r") as f:
    log = json.load(f)
...
with open("/home/work/deeptext/yys/redpen/logs/prompt_log.json", 'w') as f:
